...
exports.newEntry = (req, res) => {
  const sql = 'INSERT INTO expence_tracker_list (name, category, cost, quantity) VALUE (?,?,?,?)';
  const expense_data = req.body;
  connection.query(
    sql,
    [expense_data.name, expense_data.category, expense_data.cost, expense_data.quantity],
    (err, result) => {
      if (err) {
